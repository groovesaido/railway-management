...
export const getDashboardStats = async (req, res) => {
    try {
        const [revenueAgg, totalBooking, activeTrain, passengersToday] = await Promise.all([
...
export const getOccupancyByClass = async (req, res) => {
    try {
        ...
        res.json(grouped.map((g) => ({ className: g.ticketClass, value: g._count })));
    }
    catch (err) {
        res.status(500).json({ message: "Failed to fetch occupancy data" });
    }
};
export const getBookingByRoute = async (req, res) => {
    try {
        const tickets = await prisma.ticket.findMany({
            where: ACTIVE_STATUS_FILTER,
            select: {
